...
import { scrollAnimaion } from "../lib/scroll-animation";
...
const WebgiViewer = forwardRef((props, ref) => {
  ...
  const memoizedScrollAnimation = useCallback((position, target,isMobile, onUpdate) => {
    if (position && target && onUpdate) {
      scrollAnimaion(position, target,isMobile, onUpdate);
    }
  }, []);
  ...
  const setupViewer = useCallback(async () => {
    ...
    if(isMobileOrTablet){
      ...
      target.set(0,1.37,0);
      props.contentRef.ccurrent.className="mobile-or-tablet";
    }
